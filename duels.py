...
Логика:
  ...
  10. Лимит: максимум 2 активные дуэли от одного игрока одновременно.
...
MAX_THROWS       = 5
MAX_ACTIVE_DUELS = 2      # максимум активных дуэлей от одного игрока
MIN_BET          = 0.3
MAX_BET          = 10000.0
...
def _duel_card_text(duel: dict, *, extra: str = "") -> str:
    ...
    header = (
        ...
        f'<tg-emoji emoji-id="5330320040883411678">💰</tg-emoji> Ставка: <b><code>{amt:.2f}</code>Px</b>\n'
        f'<tg-emoji emoji-id="5278467510604160626">🏆</tg-emoji> Приз: <b><code>{prize:.2f}</code>Px</b>\n'
...
async def _activity_timeout(duel_id: str):
            ...
            try:
                await _bot.send_message(
                    ...
                    text=(
                        ...
                        f'<tg-emoji emoji-id="5197434882321567830">👤</tg-emoji> Ставка <code>{amount:.2f}</code>Px возвращена!'
                    ),
                    parse_mode=ParseMode.HTML
                )
            except Exception as e:
    ...
    if p1sum > p2sum:
        ...
        result_msg = (
            ...
            f'<tg-emoji emoji-id="5427168083074628963">👤</tg-emoji> Выигрыш: <code>+{prize:.2f}Px</code>PX\n'
        ...
        result_msg = (
            ...
            f" {p1t}: {p1_det} = <b>{p1sum}</b>\n"
            f" {p2t}: {p2_det} = <b>{p2sum}</b>\n\n"
            f'<tg-emoji emoji-id="5427168083074628963">👤</tg-emoji> Выигрыш: <code>+{prize:.2f}Px</code>\n'
            f" {winner_tag} побеждает {loser_tag}!"
            f"</blockquote>"
        )
        logging.info(f"[Duels] {duel_id} таймаут-победа {winner_tag}, приз {prize}")
        ...
        result_msg = (
            ...
            f" {p1t}: {p1_det} = <b>{p1sum}</b>\n"
            f" {p2t}: {p2_det} = <b>{p2sum}</b>\n\n"
            f'<tg-emoji emoji-id="5402186569006210455">👤</tg-emoji>Возврат: по <code>{refund:.2f}Px</code>каждому\n'
            f"</blockquote>"
        )
...
async def handle_duel_command(message: Message) -> None:
    ...
    # ── Лимит активных дуэлей ──────────────────────────────────────────────
    active_count = sum(
        1 for d in _duels.values()
        if d['status'] in ('waiting', 'playing')
        and d['player1'] == user_id
    )
    if active_count >= MAX_ACTIVE_DUELS:
        await message.reply(
            f"<blockquote>❌<b>Максимум {MAX_ACTIVE_DUELS} активные дуэли одновременно!\n"
            f"Используйте /del чтобы отменить дуэли без соперника.</b></blockquote>",
            parse_mode=ParseMode.HTML
        )
        return
