...
# ── Модуль шахты ──
from mine import mine_router, show_mine_menu, set_owner_fn as _mine_set, is_owner_fn as _mine_is
...
async def cmd_start(message: Message):
    ...
    set_owner(message.message_id, message.from_user.id)
...
# ── Шахта — передаём в модуль ──
@dp.callback_query(F.data == "mine")
async def cb_mine_entry(call: CallbackQuery):
    if not is_owner(call.message.message_id, call.from_user.id):
        await call.answer("🚫 Это не ваша кнопка!", show_alert=True); return
    await show_mine_menu(call)
...
async def main():
    ...
    # Фоновый watchdog шахты
    from mine import mine_watchdog
