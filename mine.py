...
# Активные прогресс-сообщения для авто-обновления
# progress_watchers[uid] = (chat_id, message_id)
progress_watchers: dict[int, tuple] = {}
...
def progress_text(uid: int) -> str:
    ...
    return (
        ...
        f'⛏  <b>Тиков пройдено:</b> <code>{done_ticks}</code> / <code>{max_ticks}</code>\n'
        f'💎  <b>Выкопано:</b> <code>{data["accumulated"]:.2f} Nox</code>\n'
        f'📦  <b>Осталось собрать:</b> ~<code>{avg_left} Nox</code>\n'
...
# ─────────────────────────────────────────
_bot_ref = None  # инжектируется из bot_menu.py
